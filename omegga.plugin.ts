...
export default class Plugin implements OmeggaPlugin<Config, Storage> {
  ...
  async init() {
    ...
    .on('cmd:playerinfo', async (name: string, ...user:string[]) => {
      ...
      on('cmd:s', async (name: string, ...args: string[]) => {
        ...
        for(let i = 0; i < playerData.empires.length; i++) {
          if(playerData.empires[i].name === playerData.selectedEmpire) {
            Omegga.broadcast(`<size="12"><color="#00ffff">>  </></><b><color="${playerData.empires[i].color}">${playerData.selectedEmpire}</></>: ${message}`)
          }
        }
      })
      ...
      /* Disabled in the Lite Version. If you want to use these, just use the full version, it actually works there.
